...
fun chooseDefaultPreviewSize(
    ...
    return outputSizeList?.run {
        ...
        return@run findBestSize(
            aspectList.toTypedArray(),
            maxWidth
        )
    }
}
...
fun composeRect2SensorRect(
    ...
    val point01 = com.jvziyaoyao.camera.flow.holder.camera.calculateOrientationOffsetToSensor(
        point = pair01,
        orientation = rotationOrientation,
    )
    val point02 = com.jvziyaoyao.camera.flow.holder.camera.calculateOrientationOffsetToSensor(
        point = pair02,
        orientation = rotationOrientation,
    )
